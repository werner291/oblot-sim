...
//        double maxT = schedule.values().stream().mapToDouble(events -> events.last().getT()).max().getAsDouble();
//
//        try {
//            // Create a temp file.
...
//                assertTrue( schedule.get(robot).last().getT() <= sampleT);
//            }
//        } else {
...
//                Event expected = schedule.get(event.getTargetId()).tailSet(event).stream().filter(event1 -> event1.getT() > sampleT).findFirst().get();
//
...
//                assertTrue(Math.abs(event.getT() - expected.getT()) < 0.000001);
//            }
//        }
//    }
//
...
//            TreeSet<Event> events = new TreeSet<>(Comparator.comparing((Event o) -> o.getT()));
//
...
//                events.add(new Event(EventType.values()[i%3], t, robot.getId()));
...
//                    assertAllOfType(events1, events1.get(0).getType());
...
//            assertEquals(event.getT(), afterT);
//
//            if (event.getType() == EventType.START_MOVING) {
...
//            assertTrue(checkAllRobots.remove(event.getTargetId()));
//        }
//
...
//                assertEquals(expectedType.get(event.getTargetId()), event.getType());
//                // Events returned by getNextEvent must occur at the same timestep.
//                assertEquals(event.getT(), events.get(0).getT());
//
//                expectedType.compute(event.getTargetId(), (robot, eventType) -> {
...
//            double eventT = events.get(0).getT();
//            assertTrue(eventT > t);
//
//            for (Event event : events) {
